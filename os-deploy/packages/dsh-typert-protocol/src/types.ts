...
// 说明：上游 protocol 在此为 cordis Context 增加 `typert` 注册表契约（非可选成员）。
// typert-generator 会因此把 protocol 当作带 surface 的包，要求它也导出 ./typert
// （见 analyzer.js hasPackageSurface / workspace.js validateExport）。本仓库的
// protocol 源码副本仅服务于 @qinwei/dsh-os-deploy 的构建期类型分析（Remote/
// TypertRemoteService 符号归属），运行时使用 npm 安装的正式 protocol 包，因此
// 这里刻意去掉该增强，避免为 protocol 生成 typert 工件。
